...
<b>
The main idea of DRY CRUD is to concentrate basic functionality of your application, like CRUD actions, uniform formatting, forms and tables into specifically extendable units. It generates various foundation classes that you are free to adapt to your application's needs. For each model, you then may transparently customize arbitrary parts or just fallback to the general behaviour. This applies not only for the controller, but also for the view templates and helpers.
</b>

A core element of DRY CRUD is the RenderInheritable module. This gives you inheritable views and partials. In the default case, a template is searched in the current controller's view folder. If it is not found there, the template with the same name from the superclass controller is used. This lookup path might be customized as well.

DRY CRUD is a Rails generator. All code resides in your application and is open for you to inspect and to extend. You may pick whatever you consider useful or adapt what is not sufficient. The generated code is all yours! There are no runtime dependencies to the dry_crud gem. Also, DRY CRUD does not want to provide a maximum of functionality that requires a lot of configuration, but rather a clean and lightweight foundation to build your application's requirements upon. This is why DRY CRUD comes as a generator and not as a Rails plugin.

DRY CRUD does not depend on any other plugins, but easily allows you to integrate them in order to unify the behaviour of your CRUD controllers.
...
controller/crud_controller.rb[link:http://codez.ch/dry_crud/classes/CrudController.html]:: Abstract controller providing basic CRUD actions in a RESTful way. This implementation mainly follows the one of the Rails scaffolding controller. Some enhancements were made to ease extendability. Several protected helper methods are there to be (optionally) overriden by subclasses.

=== Helpers:

helpers/standard_helper.rb[link:http://codez.ch/dry_crud/classes/StandardHelper.html]:: A view helper to standartize often used functions like formatting, tables, forms or action links. This helper is ideally defined in the ApplicationController. It is required to use the StandardTableBuilder and the StandardFormBuilder.

helpers/crud_helper.rb[link:http://codez.ch/dry_crud/classes/CrudHelper.html]:: A small helper for CrudController to render tables and forms with a default set of attributes.
...
lib/standard_table_builder.rb[link:http://codez.ch/dry_crud/classes/StandardTableBuilder.html]:: A simple helper to easily define tables listing several rows of the same data type.

lib/standard_form_builder.rb[link:http://codez.ch/dry_crud/classes/StandardFormBuilder.html]:: A form builder that automatically selects the corresponding input element for ActiveRecord columns. Input elements are rendered with a corresponding label by default.

lib/render_inheritable.rb[link:http://codez.ch/dry_crud/classes/RenderInheritable.html]:: Allows one to render inheritable views and partials. If no view file is found for the current controller, the corresponding file is looked up in its superclass hierarchy. Thus, only views or partials that look differently have to be overwritten.

lib/crud_callbacks.rb[link:http://codez.ch/dry_crud/classes/CrudCallbacks.html]:: Defines before and after callback hooks for create, update, save and destroy. When to execute the callbacks is in the responsibility of the clients of this module. This module is used in the CrudController to provide callbacks.

=== Tests:

test/crud_test_model.rb:: A dummy model to run CRUD tests against.

test/functionals/crud_controller_test_helper.rb[link:http://codez.ch/dry_crud/classes/CrudControllerTestHelper.html]:: A module to include into your functional tests for your crud controller subclasses. Provides a handfull of CRUD functionality tests.
