...
There is also a simple search functionality (based on SQL LIKE queries) implemented in ListController. Define an array of columns in your controller's +search_columns+ class variable to make the entries searchable by these fields:
...
{helpers/table_helper.rb}[http://codez.ch/dry_crud/TableHelper.html]:: Create tables to list multiple models with Crud::TableBuilder. Contains a standardized and a custom definable table.

{helpers/form_helper.rb}[http://codez.ch/dry_crud/FormHelper.html]:: Create forms to edit models with Crud::FormBuilder. Contains a standardized and a custom definable form.
...
{helpers/i18n_helper.rb}[http://codez.ch/dry_crud/I18nHelper.html]:: Translation helpers extending Rails' +translate+ helper to support translation inheritance over the controller class hierarchy.
