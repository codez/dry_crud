...
controller/crud_controller.rb[link:classes/CrudController.html]:: Abstract controller providing basic CRUD actions in a RESTful way. This implementation mainly follows the one of the Rails scaffolding controller. Some enhancements were made to ease extendability. Several protected helper methods are there to be (optionally) overriden by subclasses.

=== Helpers:

helpers/standard_helper.rb[link:classes/StandardHelper.html]:: A view helper to standartize often used functions like formatting, tables, forms or action links. This helper is ideally defined in the ApplicationController. It is required to use the StandardTableBuilder and the StandardFormBuilder.

helpers/crud_helper.rb[link:classes/CrudHelper.html]:: A small helper for CrudController to render tables and forms with a default set of attributes.
...
lib/standard_table_builder.rb[link:classes/StandardTableBuilder.html]:: A simple helper to easily define tables listing several rows of the same data type.

lib/standard_form_builder.rb[link:classes/StandardFormBuilder.html]:: A form builder that automatically selects the corresponding input element for ActiveRecord columns. Input elements are rendered with a corresponding label by default.

lib/render_inheritable.rb[link:classes/RenderInheritable.html]:: Allows one to render inheritable views and partials. If no view file is found for the current controller, the corresponding file is looked up in its superclass hierarchy. Thus, only views or partials that look differently have to be overwritten.

lib/crud_callbacks.rb[link:classes/CrudCallbacks.html]:: Defines before and after callback hooks for create, update, save and destroy. When to execute the callbacks is in the responsibility of the clients of this module. This module is used in the CrudController to provide callbacks.

=== Tests:

test/crud_test_model.rb:: A dummy model to run CRUD tests against.

test/functionals/crud_controller_test_helper.rb[link:classes/CrudControllerTestHelper.html]:: A module to include into your functional tests for your crud controller subclasses. Provides a handfull of CRUD functionality tests.
