...
# with names like 'format_{class}_{attr}', these methods are used for formatting.
#
...
module FormatHelper
  ...
  def format_attr(obj, attr)
    format_type_attr_method = :"format_#{obj.class.name.underscore.gsub('/', '_')}_#{attr}"
  ...
  def format_type(obj, attr)
    ...
    case column_type(obj, attr)
      ...
      when :text    then val.present? ? simple_format(h(val)) : UtilityHelper::EMPTY_STRING
